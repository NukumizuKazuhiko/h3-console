...
- **实例管理页精简**：删除顶部「连接」标签与全局开机/关机按钮（开关机统一走每个实例卡自己的按钮，避免误操作到非选中实例）；「完成后关机」开关移入「定时关机」折叠栏
- versionCode 80

## v2.08（2026-09-27）

- **修复「下载完成仍提示缺模型」真根因（实例端 h3ui_api）**：`_port()` 只认 `--port 6006` 分离写法，而启动脚本用 `--port=6006` 等号连写 → 自调端口恒为默认 8188，ComfyUI 实际在 6006 → 进程内 `/object_info` 自调、`/prompt` 提交全部连接拒绝（异常被吞）。现兼容两种写法；模型解析改为 folder_paths 进程内直读优先（object_info 兜底合并）；自调异常落盘 /root/autodl-tmp/h3ui_api.log 便于排查
- **修复无卡→有卡切换缺失**：无卡运行态点「生成视频」原直接 power_on（对运行中实例不生效，ComfyUI 仍是 --cpu）。现检测到无卡运行先关机、等关机完成再有卡开机，期间提示「正在切换到有卡模式」
- versionCode 79
...
- **无卡模式跳过公网地址获取**：实例处于 non_gpu 模式时不再调用公网转发地址获取链（不请求 detail、不探测 seetacloud 地址），消除「fail to fetch 公网地址」报错；`autoSetApi` 入口同步加守卫，所有调用点（轮询/登录后/生成前）统一生效
- versionCode 77

## v2.05（2026-09-27）

- **速度改为实例端直取**：v2.04 的速度靠两次探测字节差推算，首测必为 `--` 且受网关波动影响。现优先从 tqdm 日志行直接提取实时速度（首次探测即显示），字节差推算降级为兜底

## v2.04（2026-09-27）

- **修复下载百分比恒 0**：tqdm 进度条末行无换行符（`\r` 刷新），`tail` 输出与后续 `echo B:` 标记拼成一行，`B:` 不在行首导致正则匹配失败、字节恒 0。探测命令改为 `tail | tr '\r' '\n' | tail -n 1` + 显式 `echo` 强制换行分隔
- **新增下载速度监测**：按两次探测的字节差 / 实际间隔计算（MB/s 或 KB/s），显示在进度行；探测失败沿用上次速度，首测显示 `--`
...
- 「已配置完毕则不管」分支加 mainOk 条件；下载进度初始值与空值回退 "0G"，彻底消灭问号
- 钩子双写 h3ui_api.py（数据盘/系统盘 custom_nodes，覆盖迁移前后窗口）；下载脚本开头 mkdir models
- versionCode 73

## v2.01（2026-09-27）
- 修复：下载进度「一直是问号」——探测改为单命令多标记行（B 字节 / H 可读大小 / S 起始 epoch / T 旧日志 date 换算 epoch / D done 计数），任一探测失败沿用上次进度不再显示 ?；du -sb 精确字节算百分比
- 下载用时改为实例端数据：日志 `start epoch=` 时间戳（新版脚本）或实例端 `date -d` 换算旧日志，App 退出重进不再重新计时；h3_models_download.sh 起止行加 epoch
- 探测超时 45s→90s；done 判定双保险（tail 行匹配 + grep 计数）
- versionCode 72

## v2.00（2026-09-27）
- 修复：自动部署进度改写为状态留存（`nbDeployMsg`）+ 实例卡内嵌进度行（`cfgProg`），`renderInstCards` 每 4s 重渲染、页面切换、创作页 `pwrHint` 被其他功能（获取列表/自动关机提示/轮询清理）覆盖均不再丢进度；部署期间上述写入点全部加 `nbDeployingUuid` 守卫
- 修复：模型下载轮询改为「先探测后等待」，进入步骤 6 立即显示大小/百分比/用时，不再空转 1 分钟（此前观感为卡在第六步）
- 修复：实例卡「自动配置中」与「无卡模式」角标重叠——两标签包入 `.tagBox`（absolute 定位 flex 容器）并排显示
- versionCode 71

## v1.99（2026-09-26）
- **「一键配置」移到每个实例卡**（运行中/创建中/关机三种状态的按钮行均含该按钮），
  移除顶部工具栏的 debug 入口（`dbgCfgBtn`/`dbgDeploy` 兼容入口保留）；新实例下单后的
  自动部署逻辑不变，点击卡上按钮走同一流程
- **一键配置电源状态机**（按用户语义重排 `nbDeployRun` 八步）：
  - 关机 → **自动无卡开机**直接进入配置；
  - 有卡运行 → 先 SSH 探测：**已配置完毕（组件 4 项 + 模型齐）→ 直接提示就绪，不做任何操作**；
    未完成 → 关机 → 无卡开机续跑；
  - 无卡运行 → 保持现状直接续跑；
  - 配置完成后**复检确认无剩余未完成项**（重新探测 compOk+dlDone）→ 关机 → 有卡开机 → 验证端点
- 拆出可复用辅助函数：`nbProbe`（状态探测）、`nbEnsureSsh`（SSH 元数据+连通等待）、
  `nbPowerOffWait`/`nbPowerOnWait`（幂等开关机：已关机跳过关机调用、按需带 `payload:"non_gpu"`）；
  实例定位改为 uuid 精确匹配（缓存命中零等待，找不到才轮询，不再误回退列表第一个）
- i18n 新增 `nbDeployDetect`，`nbAllReady` 文案改为「无需配置」

## v1.98（2026-09-26）
- **实例管理布局调整**：实例选择下拉（`dlInst`）独占一行（宽度 100%，
  `.inst-w` 由 `flex:1` 改为 `width:100%`），「租用 / 获取列表 / 一键配置」按钮另起一行，
  解决按钮旁被压缩的问题

## v1.97（2026-09-26）
- **修正 v1.96 误解**：开屏「选择使用模式」屏恢复原双卡片样式；改为把**设置页**的
  「使用模式」（原「-」文本 + 「重新选择」按钮）换成 Tom Select 下拉栏（新手/老手，
  即改即生效，与语言/主题选择器同款皮肤），选项文案随语言切换刷新
- 移除 `modeVal` 文本与「重新选择」按钮依赖；`applyLevel` 改为同步 `levelSel` 值
